...
# spt's Subset stores a Trainer reference to stamp samples with global_step/current_epoch. 
# Nothing here reads those, and a Trainer holds thread locks — so the dataset becomes unpicklable and spawn-based dataloader workers fail. 
# Disabling the setter is why num_workers>0 works at all.
Subset.set_pl_trainer = lambda self, trainer: None
...
#This was taken from Lewm.py
#It wraps self.optimizers to always return a list, which is needed since we may use only one worker.
...
def run(cfg):
    ...
    world_model = spt.Module(
        ...
        optim={
            ...
            'scheduler': dict(cfg.scheduler) #My ADDITION
        },
        },
    )
    ...
    #CHANGED FROM logger to wandblogger to avoid error of local variable 'logger' referenced before assignment
    wandblogger = None
    if cfg.wandb.enabled:
        wandblogger = WandbLogger(**cfg.wandb.config)
        wandblogger.log_hyperparams(OmegaConf.to_container(cfg))
